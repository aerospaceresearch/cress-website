...
docker run --name cress-nginx --net="host" --volumes-from cress-data -p 80:80 -v `pwd`/nginx.conf:/etc/nginx/nginx.conf -v /var/lib/acme:/var/lib/acme --restart=always -d nginx
